// Declares learner material contracts and endpoint calls, turning a teacher draft into the multipart upload the backend expects and linking a material to a piece.

import type { PersonaRole } from "./contracts";
import { apiRequest } from "./transport";

export type MaterialAttachment = { name: string; kind: "image" | "pdf"; url: string };
export type Material = { id: string; assignment: string; title: string; body: string; piece: string | null; attachments: MaterialAttachment[]; created_at: string };
export type MaterialDraft = { title: string; body: string; files: File[]; piece?: string };
...
export function createMaterial(assignmentId: string, draft: MaterialDraft) {
  ...
  if (draft.piece) form.append("piece", draft.piece);
...
// An empty piece removes the link.
export function pinMaterial(id: string, piece: string) {
  return apiRequest<Material>(`/api/teachers/materials/${encodeURIComponent(id)}`, { method: "PATCH", body: { piece: piece || null } });
}
